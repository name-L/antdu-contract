<template>
    <div class="department-data-picture">
...
import chooseDepartmentPartyAData from './choose-department-partyA-data.vue';
import chooseDepartmentPartyBData from './choose-department-partyB-data.vue';
...
export default {
    data () {
        return {
            ...
            loading: false,
            partyADepartment: '选中部门甲方单位合同数据情况',
            partyBDepartment: '选中部门乙方单位合同数据情况',
    ...
    methods: {
        ...
        getDepartmentData (name, item) {
            axios.get(`/contract/department-data`, {
                params: {
                    department: name,
                    item: item
                }
    ...
    mounted () {
        var chartDom = document.getElementById('chart-dom');
        var myChart = echarts.init(chartDom);
        myChart.showLoading();
        axios.get(`/contract/department-statistics`, {
            params: {
                pageSize: 20
            }
        }).then(res => {
            myChart.hideLoading();
            this.chartList = res.data.data.records;
            this.getDepartmentData(this.chartList[0].department, 'party_a');
            this.getDepartmentData(this.chartList[0].department, 'party_b');
            this.name = this.chartList[0].department;
            var option;

            let dataAxis = [];
            let data = [];
            this.chartList.forEach(item => {
                dataAxis.push(item.department);
                ...
                series: [
                    {
                        ...
                        encode: {
                            ...
                            y: 'product',
                            tooltip: ['amount', 'product']
                        },
                        itemStyle: {
                            color: (params) => {
                                var key = params.dataIndex;
                                // 判断渲染的index === 点击的index 给不同颜色
                                if (key === this.index) {
            ...
            myChart.on('click', params => {
                // 不是点击柱子的时候没有params.dataIndex so 点击name的时候储存index
                let ind;
                ...
                // 判断储存的index 是否===点击index||点击柱子name 储存的ind
                if (this.index === params.dataIndex || this.index === ind) {
                    this.partyADepartment = '全国部门甲方单位合同数据情况';
                    this.partyBDepartment = '全国部门乙方单位合同数据情况';
                    this.getDepartmentData('all', 'party_a');
                    this.getDepartmentData('all', 'party_b');
                    this.name = 'all';
                    this.index = '';
                } else {
                    this.partyADepartment = '选中部门甲方单位合同数据情况';
                    this.partyBDepartment = '选中部门乙方单位合同数据情况';
                    this.getDepartmentData(params.name || params.value, 'party_a');
                    this.getDepartmentData(params.name || params.value, 'party_b');
                    this.name = params.name || params.value;
                    // 点击储存index || 点击name的时候没有params.dataIndex就储存的ind
                    this.index = params.dataIndex || ind;
                }
            });
...
<style lang="less" scoped src="./department-data-picture.less">
</style>